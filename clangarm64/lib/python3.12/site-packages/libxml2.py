...
def htmlHandleOmittedElem(val):
    """DEPRECATED: Use HTML_PARSE_NOIMPLIED  Set and return the
       previous value for handling HTML omitted tags. """
...
def htmlNewParserCtxt():
    """Allocate and initialize a new HTML parser context.  This
...
def htmlReadFile(filename, encoding, options):
    """Convenience function to parse an HTML file from the
      filesystem, the network or a global user-defined resource
       loader.  See htmlCtxtReadFile for details. """
...
def htmlReadMemory(buffer, size, url, encoding, options):
    """Convenience function to parse an HTML document from memory.
      The input buffer must not contain any terminating null
       bytes.  See htmlCtxtReadMemory for details. """
...
def htmlIsBooleanAttr(name):
    """DEPRECATED: Internal function, don't use.  Determine if a
       given attribute is a boolean attribute. """
...
def SAXDefaultVersion(version):
    """DEPRECATED: Use parser option XML_PARSE_SAX1.  Has no
       effect. """
...
def defaultSAXHandlerInit():
    """DEPRECATED: This function is a no-op. Call xmlInitParser to
      initialize the library.  Initialize the default SAX2 handler """
...
def addEncodingAlias(name, alias):
    """DEPRECATED: This function modifies global state and is not
      thread-safe.  Registers an alias @alias for an encoding
       named @name. Existing alias will be overwritten. """
...
def cleanupEncodingAliases():
    """DEPRECATED: This function modifies global state and is not
       thread-safe.  Unregisters all aliases """
    libxml2mod.xmlCleanupEncodingAliases()

def delEncodingAlias(alias):
    """DEPRECATED: This function modifies global state and is not
       thread-safe.  Unregisters an encoding alias @alias """
    ret = libxml2mod.xmlDelEncodingAlias(alias)
    return ret

def encodingAlias(alias):
    """DEPRECATED: This function is not thread-safe.  Lookup an
       encoding name for the given alias. """
...
def createDocParserCtxt(str):
    """Creates a parser context for an XML in-memory document. """
    ret = libxml2mod.xmlCreateDocParserCtxt(str)
...
def initParser():
    """Initialization function for the XML parser.  For older
      versions, it's recommended to call this function once from
      the main thread before using the library in multithreaded
      programs.  Since 2.14.0, there's no distinction between
       threads. It should be unnecessary to call this function. """
    libxml2mod.xmlInitParser()

def keepBlanksDefault(val):
    """DEPRECATED: Use the modern options API with
...
def parseDoc(cur):
    """DEPRECATED: Use xmlReadDoc.  parse an XML in-memory
       document and build a tree. """
...
def parseFile(filename):
    """DEPRECATED: Use xmlReadFile.  parse an XML file and build a
...
def parseMemory(buffer, size):
    """DEPRECATED: Use xmlReadMemory.  parse an XML in-memory
       block and build a tree. """
...
def readDoc(cur, URL, encoding, options):
    """Convenience function to parse an XML document from a
       zero-terminated string.  See xmlCtxtReadDoc for details. """
...
def readFd(fd, URL, encoding, options):
    """Parse an XML from a file descriptor and build a tree.  See
      xmlCtxtReadFd for details.  NOTE that the file descriptor
       will not be closed when the context is freed or reset. """
...
def readFile(filename, encoding, options):
    """Convenience function to parse an XML file from the
      filesystem, the network or a global user-define resource
      loader.  This function always enables the XML_PARSE_UNZIP
      option for backward compatibility. If a "-" filename is
      passed, it will read from stdin. Both of these features are
      potentially insecure and might be removed from later
       versions.  See xmlCtxtReadFile for details. """
...
def recoverFile(filename):
    """DEPRECATED: Use xmlReadFile with XML_PARSE_RECOVER.  parse
...
def substituteEntitiesDefault(val):
    """DEPRECATED: Use the modern options API with
      ...
      default parser behavior SAX::substituteEntities() has to be
       used for changing that on a file by file basis. """
...
def checkLanguageID(lang):
    """DEPRECATED: Internal function, do not use.  Checks that the
...
def copyCharMultiByte(out, val):
    """DEPRECATED: Internal function, don't use.  append the char
       value in the array """
    ret = libxml2mod.xmlCopyCharMultiByte(out, val)
    return ret

def createEntityParserCtxt(URL, ID, base):
    """DEPRECATED: Don't use.  Create a parser context for an
...
def createMemoryParserCtxt(buffer, size):
    """Create a parser context for an XML in-memory document. The
       input buffer must not contain a terminating null byte. """
...
def isLetter(c):
    """DEPRECATED: Internal function, don't use.  Check whether
      the character is allowed by the production [84] Letter ::=
...
def compressMode():
    """DEPRECATED: Use xmlGetDocCompressMode  get the default
       compression mode used, ZLIB based. """
...
def newComment(content):
    """Use of this function is DISCOURAGED in favor of
       xmlNewDocComment.  Create a comment node. """
    ret = libxml2mod.xmlNewComment(content)
    if ret is None:raise treeError('xmlNewComment() failed')
    return xmlNode(_obj=ret)

def newDoc(version):
    """Creates a new XML document. If version is None, "1.0" is
       used. """
...
def newPI(name, content):
    """Create a processing instruction node.  Use of this function
       is DISCOURAGED in favor of xmlNewDocPI. """
    ret = libxml2mod.xmlNewPI(name, content)
    if ret is None:raise treeError('xmlNewPI() failed')
    return xmlNode(_obj=ret)

def newText(content):
    """Create a text node.  Use of this function is DISCOURAGED in
       favor of xmlNewDocText. """
...
def setCompressMode(mode):
    """DEPRECATED: Use xmlSetDocCompressMode  set the default
      compression mode used, ZLIB based Correct values: 0
       (uncompressed) to 9 (max compression) """
...
def URIEscapeStr(str, list):
    """This routine escapes a string to hex, ignoring unreserved
...
def canonicPath(path):
    """Prepares a path.  If the path contains the substring "://",
...
def checkFilename(path):
    """DEPRECATED: Internal function, don't use.  if stat is not
...
def iOHTTPMatch(filename):
    """DEPRECATED: Internal function, don't use.  check if the URI
       matches an HTTP one """
...
def schemaInitTypes():
    ...
    ret = libxml2mod.xmlSchemaInitTypes()
    return ret
...
class xmlNode(xmlCore):
    ...
    def addChild(self, cur):
        """Unlink @cur and append it to the children of @parent.  If
          ...
          @cur, the old attribute is destroyed.  General notes:  Move
          operations like xmlAddChild can cause element or attribute
          nodes to reference namespaces that aren't declared in one
          of their ancestors. This can lead to use-after-free errors
          if the elements containing the declarations are freed
          later, especially when moving nodes from one document to
          another. You should consider calling xmlReconciliateNs
          after a move operation to normalize namespaces. Another
          option is to call xmlDOMWrapAdoptNode with the target
          parent before moving a node.  For the most part, move
          operations don't check whether the resulting tree structure
          is valid. Users must make sure that parent nodes only
          receive children of valid types. Inserted child nodes must
          never be an ancestor of the parent node to avoid cycles in
          the tree structure. In general, only document, document
          fragments, elements and attributes should be used as parent
          nodes.  When moving a node between documents and a memory
          allocation fails, the node's content will be corrupted and
          it will be unlinked. In this case, the node must be freed
           manually.  Moving DTDs between documents isn't supported. """
    ...
    def addContent(self, content):
        ...
        ret = libxml2mod.xmlNodeAddContent(self._o, content)
        return ret
    ...
    def addContentLen(self, content, len):
        """Append the extra substring to the node content. NOTE: In
          ...
          be raw text, so unescaped XML special chars are allowed,
           entity references are not supported. """
        ret = libxml2mod.xmlNodeAddContentLen(self._o, content, len)
        return ret

    def addNextSibling(self, cur):
        """Unlinks @cur and inserts it as next sibling after @prev. 
          ...
          attribute with a name matching @cur, the old attribute is
           destroyed.  See the notes in xmlAddChild. """
        if cur is None: cur__o = None
        else: cur__o = cur._o
        ret = libxml2mod.xmlAddNextSibling(self._o, cur__o)
        if ret is None:raise treeError('xmlAddNextSibling() failed')
        __tmp = xmlNode(_obj=ret)
        return __tmp

    def addPrevSibling(self, cur):
        """Unlinks @cur and inserts it as previous sibling before
        ...
        ret = libxml2mod.xmlAddPrevSibling(self._o, cur__o)
        if ret is None:raise treeError('xmlAddPrevSibling() failed')
        __tmp = xmlNode(_obj=ret)
        return __tmp

    def addSibling(self, cur):
        """Unlinks @cur and inserts it as last sibling of @node.  If
          @cur is a text node, it may be merged with an adjacent text
          node and freed. In this case the text node containing the
          merged content is returned.  If @cur is an attribute node,
          ...
          matching @cur, the old attribute is destroyed.  See the
           notes in xmlAddChild. """
        if cur is None: cur__o = None
        else: cur__o = cur._o
        ret = libxml2mod.xmlAddSibling(self._o, cur__o)
        if ret is None:raise treeError('xmlAddSibling() failed')
        __tmp = xmlNode(_obj=ret)
        return __tmp

    def copyNode(self, extended):
        """Copy a node.  Use of this function is DISCOURAGED in favor
           of xmlDocCopyNode. """
    ...
    def copyNodeList(self):
        """Copy a node list and all children.  Use of this function is
           DISCOURAGED in favor of xmlDocCopyNodeList. """
    ...
    def copyProp(self, cur):
        """Create a copy of the attribute. This function sets the
          ...
          set the attribute by calling xmlAddChild afterwards or
           reset the parent pointer to None. """
        if cur is None: cur__o = None
        else: cur__o = cur._o
    ...
    def copyPropList(self, cur):
        """Create a copy of an attribute list. This function sets the
          parent pointers of the copied attributes to @target but
           doesn't set the attributes on the target element. """
        if cur is None: cur__o = None
        else: cur__o = cur._o
    ...
    def docSetRootElement(self, doc):
        """Set the root element of the document (doc->children is a
          list containing possibly comments, PIs, etc ...).  @root
           must be an element node. It is unlinked before insertion. """
        if doc is None: doc__o = None
        else: doc__o = doc._o
    ...
    def firstElementChild(self):
        """Find the first child node which is an element.  Note that
    ...
    def freeNode(self):
        """Free a node including all the children.  This doesn't
    ...
    def getBase(self, doc):
        """See xmlNodeGetBaseSafe. This function doesn't allow to
    ...
    def lastElementChild(self):
        """Find the last child node which is an element.  Note that
           entity references are not expanded. """
    ...
    def listGetRawString(self, doc, inLine):
        """Serializes attribute children (text and entity reference
          nodes) into a string.  If @inLine is true, entity
          references will be substituted. Otherwise, entity
          references will be kept and special characters like '&'
           will be escaped. """
        if doc is None: doc__o = None
        else: doc__o = doc._o
    ...
    def listGetString(self, doc, inLine):
        """Serializes attribute children (text and entity reference
          nodes) into a string.  If @inLine is true, entity
          references will be substituted. Otherwise, entity
          references will be kept and special characters like '&' as
          well as non-ASCII chars will be escaped. See
    ...
    def newChild(self, ns, name, content):
        """Create a new child element and append it to a parent
    ...
    def newNs(self, href, prefix):
        """Create a new namespace. For a default namespace, @prefix
    ...
    def newNsPropEatName(self, ns, name, value):
        """Like xmlNewNsProp, but the @name string will be used
          directly without making a copy. Takes ownership of @name
           which will also be freed on error. """
        if ns is None: ns__o = None
        else: ns__o = ns._o
    ...
    def newProp(self, name, value):
        """Create an attribute node.  If provided, @value should be a
    ...
    def nsProp(self, name, nameSpace):
        """Search and get the value of an attribute associated to a
          ...
          distinguish malloc failures from missing attributes. It's
           more robust to use xmlNodeGetAttrValue. """
    ...
    def replaceNode(self, cur):
        """Unlink the old node. If @cur is provided, it is unlinked
    ...
    def setBase(self, uri):
        ...
        ret = libxml2mod.xmlNodeSetBase(self._o, uri)
        return ret

    def setContent(self, content):
        """Replace the text content of a node.  Sets the raw text
        ...
        ret = libxml2mod.xmlNodeSetContent(self._o, content)
        return ret

    def setContentLen(self, content, len):
        """See xmlNodeSetContent. """
        ret = libxml2mod.xmlNodeSetContentLen(self._o, content, len)
        return ret

    def setLang(self, lang):
        """Set the language of a node, i.e. the values of the xml:lang
           attribute. """
        ret = libxml2mod.xmlNodeSetLang(self._o, lang)
        return ret
    ...
    def setListDoc(self, doc):
        ...
        ret = libxml2mod.xmlSetListDoc(self._o, doc__o)
        return ret
    ...
    def setNs(self, ns):
        """Set the namespace of an element or attribute node. Passing
           a None namespace unsets the namespace. """
        if ns is None: ns__o = None
        else: ns__o = ns._o
    ...
    def setSpacePreserve(self, val):
        ...
        ret = libxml2mod.xmlNodeSetSpacePreserve(self._o, val)
        return ret
    ...
    def setTreeDoc(self, doc):
        """This is an internal function which shouldn't be used. It is
          ...
          document.  Also copy strings from the old document's
           dictionary and remove ID attributes from the old ID table. """
        if doc is None: doc__o = None
        else: doc__o = doc._o
        ret = libxml2mod.xmlSetTreeDoc(self._o, doc__o)
        return ret
    ...
    def textMerge(self, second):
        """Merge the second text node into the first. If @first is
          None, @second is returned. Otherwise, the second node is
           unlinked and freed. """
    ...
    def unlinkNode(self):
        """Unlink a node from its tree.  The node is not freed. Unless
          it is reinserted, it must be managed manually and freed
           eventually by calling xmlFreeNode. """
    ...
    def validNormalizeAttributeValue(self, doc, name, value):
        """DEPRECATED: Internal function, don't use.  Does the
...
class xmlDoc(xmlNode):
    ...
    def htmlAutoCloseTag(self, name, elem):
        """DEPRECATED: Internal function, don't use.  The HTML DTD
          allows a tag to implicitly close other tags. The list is
          kept in htmlStartClose array. This function checks if the
    ...
    def htmlIsAutoClosed(self, elem):
        """DEPRECATED: Internal function, don't use.  The HTML DTD
          allows a tag to implicitly close other tags. The list is
          kept in htmlStartClose array. This function checks if a tag
           is autoclosed by one of it's child """
    ...
    def encodeEntitiesReentrant(self, input):
        """Do a global encoding of a string, replacing the predefined
          ...
          routine is reentrant, and result must be deallocated.  This
          escapes '<', '>', '&' and '\r'. If the document has no
          encoding, non-ASCII codepoints are escaped. There is some
           special handling for HTML documents. """
    ...
    def encodeSpecialChars(self, input):
        """Do a global encoding of a string, replacing the predefined
          ...
          deallocated.  This escapes '<', '>', '&', '"' and '\r'
           chars. """
    ...
    def ctxtValidateDocument(self, ctxt):
        """Validate a document.  Like xmlValidateDocument but uses the
          parser context's error handler.  Option XML_PARSE_DTDLOAD
          should be enabled in the parser context to make external
           entities work.  Availabe since 2.14.0. """
        if ctxt is None: ctxt__o = None
        else: ctxt__o = ctxt._o
        ret = libxml2mod.xmlCtxtValidateDocument(ctxt__o, self._o)
        return ret

    def ctxtValidateDtd(self, ctxt, dtd):
        """Validate a document against a DTD.  Like xmlValidateDtd but
          uses the parser context's error handler.  Availabe since
           2.14.0. """
        if ctxt is None: ctxt__o = None
        else: ctxt__o = ctxt._o
        if dtd is None: dtd__o = None
        else: dtd__o = dtd._o
        ret = libxml2mod.xmlCtxtValidateDtd(ctxt__o, self._o, dtd__o)
        return ret
    ...
    def copyDoc(self, recursive):
        """Copy a document. If recursive, the content tree will be
           copied too as well as DTD, namespaces and entities. """
    ...
    def newCharRef(self, name):
        """This function is MISNAMED. It doesn't create a character
    ...
    def newDocNode(self, ns, name, content):
        """Create an element node.  If provided, @content is expected
          ...
          tree. Note that a document only owns nodes reachable from
           the root node. Unlinked subtrees must be freed manually. """
        if ns is None: ns__o = None
        else: ns__o = ns._o
    ...
    def newDocNodeEatName(self, ns, name, content):
        """Create an element node.  Like xmlNewDocNode, but the @name
          string will be used directly without making a copy. Takes
           ownership of @name which will also be freed on error. """
        if ns is None: ns__o = None
        else: ns__o = ns._o
    ...
    def newDocProp(self, name, value):
        """Create an attribute object.  If provided, @value is
    ...
    def newDocRawNode(self, ns, name, content):
        """Create an element node.  If provided, @value should be a
    ...
    def newDtd(self, name, ExternalID, SystemID):
        """Create a DTD node.  If a document is provided, it is an
    ...
    def newReference(self, name):
        """Create a new entity reference node, linking the result with
    ...
    def setListDoc(self, list):
        ...
        ret = libxml2mod.xmlSetListDoc(list__o, self._o)
        return ret
    ...
    def setTreeDoc(self, tree):
        ...
        ret = libxml2mod.xmlSetTreeDoc(tree__o, self._o)
        return ret

    def stringGetNodeList(self, value):
        """DEPRECATED: Use xmlNodeSetContent.  Parse an attribute
          ...
          errors or references to undeclared entities will be ignored
           silently with unspecified results. """
    ...
    def stringLenGetNodeList(self, value, len):
        """DEPRECATED: Use xmlNodeSetContentLen.  See
    ...
    def validateDocument(self, vctxt):
        """DEPRECATED: This function can't report malloc or other
          failures. Use xmlCtxtValidateDocument.  Try to validate the
          document instance  basically it does the all the checks
          described by the XML Rec i.e. validates the internal and
          external subset (if present) and validate the document tree. """
        if vctxt is None: vctxt__o = None
        else: vctxt__o = vctxt._o
        ret = libxml2mod.xmlValidateDocument(vctxt__o, self._o)
        return ret

    def validateDocumentFinal(self, ctxt):
        """DEPRECATED: Internal function, don't use.  Does the final
    ...
    def validateElement(self, ctxt, root):
        """Try to validate the subtree under an element """
        if ctxt is None: ctxt__o = None
        else: ctxt__o = ctxt._o
        if root is None: root__o = None
        else: root__o = root._o
        ret = libxml2mod.xmlValidateElement(ctxt__o, self._o, root__o)
        return ret

    def validateNotationUse(self, ctxt, notationName):
        """DEPRECATED: Internal function, don't use.  Validate that
    ...
    def validateOneAttribute(self, ctxt, elem, attr, value):
        """DEPRECATED: Internal function, don't use.  Try to validate
    ...
    def validatePopElement(self, ctxt, elem, qname):
        """DEPRECATED: Internal function, don't use.  Pop the element
           end from the validation stack. """
        if ctxt is None: ctxt__o = None
        else: ctxt__o = ctxt._o
        if elem is None: elem__o = None
        else: elem__o = elem._o
    ...
    def validatePushElement(self, ctxt, elem, qname):
        """DEPRECATED: Internal function, don't use.  Push a new
           element start on the validation stack. """
        if ctxt is None: ctxt__o = None
        else: ctxt__o = ctxt._o
        if elem is None: elem__o = None
        else: elem__o = elem._o
...
class parserCtxt(parserCtxtCore):
    ...
    def htmlCtxtReadDoc(self, str, URL, encoding, options):
        """Parse an HTML in-memory document and build a tree.  See
           htmlCtxtUseOptions for details. """
        ret = libxml2mod.htmlCtxtReadDoc(self._o, str, URL, encoding, options)
        if ret is None:raise treeError('htmlCtxtReadDoc() failed')
        __tmp = xmlDoc(_obj=ret)
        return __tmp

    def htmlCtxtReadFd(self, fd, URL, encoding, options):
        """Parse an HTML from a file descriptor and build a tree.  See
    ...
    def htmlCtxtReadFile(self, filename, encoding, options):
        """Parse an HTML file from the filesystem, the network or a
          user-defined resource loader.  See htmlCtxtUseOptions for
           details. """
    ...
    def htmlCtxtReadMemory(self, buffer, size, URL, encoding, options):
        """Parse an HTML in-memory document and build a tree. The
          input buffer must not contain any terminating null bytes. 
    ...
    def htmlCtxtSetOptions(self, options):
        """Applies the options to the parser context. Unset options
          are cleared.  Available since 2.14.0. With older versions,
          you can use htmlCtxtUseOptions.  HTML_PARSE_RECOVER  No
          effect as of 2.14.0.  HTML_PARSE_HTML5  Make the tokenizer
          emit a SAX callback for each token. This results in
          unbalanced invocations of startElement and endElement.  For
          now, this is only usable with custom SAX callbacks. 
          HTML_PARSE_NODEFDTD  Do not default to a doctype if none
          was found.  HTML_PARSE_NOERROR  Disable error and warning
          reports to the error handlers. Errors are still accessible
          with xmlCtxtGetLastError.  HTML_PARSE_NOWARNING  Disable
          warning reports.  HTML_PARSE_PEDANTIC  No effect. 
          HTML_PARSE_NOBLANKS  Remove some text nodes containing only
          whitespace from the result document. Which nodes are
          removed depends on a conservative heuristic. The
          reindenting feature of the serialization code relies on
          this option to be set when parsing. Use of this option is
          DISCOURAGED.  HTML_PARSE_NONET  No effect. 
          HTML_PARSE_NOIMPLIED  Do not add implied html, head or body
          elements.  HTML_PARSE_COMPACT  Store small strings directly
          in the node struct to save memory.  HTML_PARSE_HUGE  Relax
          some internal limits.  Available since 2.14.0. Use
          XML_PARSE_HUGE works with older versions.  Maximum size of
          text nodes, tags, comments, CDATA sections  normal: 10M
          huge:    1B  Maximum size of names, system literals, pubid
          literals  normal: 50K huge:   10M  Maximum nesting depth of
          elements  normal:  256 huge:   2048  HTML_PARSE_IGNORE_ENC 
          Ignore the encoding in the HTML declaration. This option is
          mostly unneeded these days. The only effect is to enforce
          UTF-8 decoding of ASCII-like data.  HTML_PARSE_BIG_LINES 
          Enable reporting of line numbers larger than 65535. 
           Available since 2.14.0. """
        ret = libxml2mod.htmlCtxtSetOptions(self._o, options)
        return ret

    def htmlCtxtUseOptions(self, options):
        """DEPRECATED: Use htmlCtxtSetOptions.  Applies the options to
          the parser context. The following options are never cleared
          and can only be enabled:  HTML_PARSE_NODEFDTD
          HTML_PARSE_NOERROR HTML_PARSE_NOWARNING
          HTML_PARSE_NOIMPLIED HTML_PARSE_COMPACT HTML_PARSE_HUGE
           HTML_PARSE_IGNORE_ENC HTML_PARSE_BIG_LINES """
    ...
    def htmlParseChunk(self, chunk, size, terminate):
        """Parse a chunk of memory in push parser mode.  Assumes that
    ...
    def byteConsumed(self):
        """DEPRECATED: Don't use.  This function provides the current
          index of the parser relative to the start of the current
          entity. This function is computed in bytes from the
          beginning starting at zero and finishing at the size in
          byte of the file if parsing a file. The function is of
          constant cost if the input is UTF-8 but can be costly if
           run on non-UTF-8 input. """
    ...
    def ctxtGetDeclaredEncoding(self):
        """Available since 2.14.0. """
        ret = libxml2mod.xmlCtxtGetDeclaredEncoding(self._o)
        return ret

    def ctxtGetDocument(self):
        """Available since 2.14.0. """
        ret = libxml2mod.xmlCtxtGetDocument(self._o)
        if ret is None:raise treeError('xmlCtxtGetDocument() failed')
        __tmp = xmlDoc(_obj=ret)
        return __tmp

    def ctxtGetOptions(self):
        """Get the current options of the parser context.  Available
           since 2.14.0. """
        ret = libxml2mod.xmlCtxtGetOptions(self._o)
        return ret

    def ctxtGetStandalone(self):
        """Available since 2.14.0. """
        ret = libxml2mod.xmlCtxtGetStandalone(self._o)
        return ret

    def ctxtGetValidCtxt(self):
        """Available since 2.14.0. """
        ret = libxml2mod.xmlCtxtGetValidCtxt(self._o)
        if ret is None:raise treeError('xmlCtxtGetValidCtxt() failed')
        __tmp = ValidCtxt(_obj=ret)
        return __tmp

    def ctxtGetVersion(self):
        """Available since 2.14.0. """
        ret = libxml2mod.xmlCtxtGetVersion(self._o)
        return ret

    def ctxtIsHtml(self):
        """Available since 2.14.0. """
        ret = libxml2mod.xmlCtxtIsHtml(self._o)
        return ret

    def ctxtIsStopped(self):
        """Available since 2.14.0. """
        ret = libxml2mod.xmlCtxtIsStopped(self._o)
        return ret

    def ctxtReadDoc(self, str, URL, encoding, options):
        """Parse an XML in-memory document and build a tree.  @URL is
          used as base to resolve external entities and for error
           reporting.  See xmlCtxtUseOptions for details. """
        ret = libxml2mod.xmlCtxtReadDoc(self._o, str, URL, encoding, options)
        if ret is None:raise treeError('xmlCtxtReadDoc() failed')
        __tmp = xmlDoc(_obj=ret)
        return __tmp

    def ctxtReadFd(self, fd, URL, encoding, options):
        """Parse an XML document from a file descriptor and build a
    ...
    def ctxtReadFile(self, filename, encoding, options):
        """Parse an XML file from the filesystem, the network or a
          user-defined resource loader.  This function always enables
          the XML_PARSE_UNZIP option for backward compatibility. This
          feature is potentially insecure and might be removed from
           later versions. """
    ...
    def ctxtSetOptions(self, options):
        """Applies the options to the parser context. Unset options
          ...
          Not supported by the push parser.  XML_PARSE_NOENT  Despite
          the confusing name, this option enables substitution of
          entities. The resulting tree won't contain any entity
          reference nodes.  This option also enables loading of
          external entities (both general and parameter entities)
          which is dangerous. If you process untrusted data, it's
          recommended to set the XML_PARSE_NO_XXE option to disable
          loading of external entities.  XML_PARSE_DTDLOAD  Enables
          loading of an external DTD and the loading and substitution
          of external parameter entities. Has no effect if
          XML_PARSE_NO_XXE is set.  XML_PARSE_DTDATTR  Adds default
          attributes from the DTD to the result document.  Implies
          XML_PARSE_DTDLOAD, but loading of external content can be
          disabled with XML_PARSE_NO_XXE.  XML_PARSE_DTDVALID  This
          option enables DTD validation which requires to load
          external DTDs and external entities (both general and
          parameter entities) unless XML_PARSE_NO_XXE was set. 
          XML_PARSE_NO_XXE  Disables loading of external DTDs or
          entities.  Available since 2.13.0.  XML_PARSE_NOERROR 
          Disable error and warning reports to the error handlers.
          Errors are still accessible with xmlCtxtGetLastError. 
          XML_PARSE_NOWARNING  Disable warning reports. 
          XML_PARSE_PEDANTIC  Enable some pedantic warnings. 
          XML_PARSE_NOBLANKS  Remove some whitespace from the result
          document. Where to remove whitespace depends on DTD element
          declarations or a broken heuristic with unfixable bugs. Use
          of this option is DISCOURAGED.  Not supported by the push
          parser.  XML_PARSE_SAX1  Always invoke the deprecated SAX1
          startElement and endElement handlers. This option is
          DEPRECATED.  XML_PARSE_NONET  Disable network access with
          the builtin HTTP client.  XML_PARSE_NODICT  Create a
          document without interned strings, making all strings
          separate memory allocations.  XML_PARSE_NSCLEAN  Remove
          redundant namespace declarations from the result document. 
          ...
          line numbers larger than 65535.  XML_PARSE_UNZIP  Enable
          input decompression. Setting this option is discouraged to
          avoid zip bombs.  Available since 2.14.0. 
          XML_PARSE_NO_SYS_CATALOG  Disables the global system XML
          catalog.  Available since 2.14.0.  XML_PARSE_CATALOG_PI 
          Enable XML catalog processing instructions.  Available
           since 2.14.0. """
        ret = libxml2mod.xmlCtxtSetOptions(self._o, options)
        return ret

    def ctxtUseOptions(self, options):
        """DEPRECATED: Use xmlCtxtSetOptions.  Applies the options to
          the parser context. The following options are never cleared
    ...
    def ctxtValidateDocument(self, doc):
        """Validate a document.  Like xmlValidateDocument but uses the
          parser context's error handler.  Option XML_PARSE_DTDLOAD
          should be enabled in the parser context to make external
           entities work.  Availabe since 2.14.0. """
        ...
        ret = libxml2mod.xmlCtxtValidateDocument(self._o, doc__o)
        return ret

    def ctxtValidateDtd(self, doc, dtd):
        """Validate a document against a DTD.  Like xmlValidateDtd but
          uses the parser context's error handler.  Availabe since
           2.14.0. """
        if doc is None: doc__o = None
        ...
        ret = libxml2mod.xmlCtxtValidateDtd(self._o, doc__o, dtd__o)
        return ret

    def initParserCtxt(self):
        """DEPRECATED: Internal function which will be made private in
           a future version.  Initialize a parser context """
    ...
    def parseChunk(self, chunk, size, terminate):
        """Parse a chunk of memory in push parser mode.  Assumes that
          ...
          will free the document in ctxt->myDoc. So ctxt->myDoc
           should be set to None after extracting the document. """
        ret = libxml2mod.xmlParseChunk(self._o, chunk, size, terminate)
        return ret

    def parseDocument(self):
        """Parse an XML document and invoke the SAX handlers. This is
          useful if you're only interested in custom SAX callbacks.
           If you want a document tree, use xmlCtxtParseDocument. """
        ret = libxml2mod.xmlParseDocument(self._o)
        return ret

    def parseExtParsedEnt(self):
        """DEPRECATED: Internal function, don't use.  parse a general
          parsed entity An external general parsed entity is
    ...
    def ctxtErrMemory(self):
        """Handle an out-of-memory error.  Available since 2.13.0. """
        libxml2mod.xmlCtxtErrMemory(self._o)

    def nextChar(self):
        """DEPRECATED: Internal function, do not use.  Skip to the
           next char input char. """
        libxml2mod.xmlNextChar(self._o)

    def parseAttValue(self):
        """DEPRECATED: Internal function, don't use.  parse a value
    ...
    def parseAttributeListDecl(self):
        """DEPRECATED: Internal function, don't use.  Parse an
    ...
    def parseContent(self):
        """Parse XML element content. This is useful if you're only
          interested in custom SAX callbacks. If you want a node
           list, use xmlCtxtParseContent. """
        libxml2mod.xmlParseContent(self._o)

    def parseDocTypeDecl(self):
        """DEPRECATED: Internal function, don't use.  parse a DOCTYPE
    ...
    def parseElementDecl(self):
        """DEPRECATED: Internal function, don't use.  Parse an element
    ...
    def parseEncName(self):
        """DEPRECATED: Internal function, don't use.  parse the XML
    ...
    def parseEndTag(self):
        """DEPRECATED: Internal function, don't use.  parse an end of
          tag  [42] ETag ::= '</' Name S? '>'  With namespace  [NS 9]
           ETag ::= '</' QName S? '>' """
        libxml2mod.xmlParseEndTag(self._o)

    def parseEntityDecl(self):
        """DEPRECATED: Internal function, don't use.  Parse an entity
    ...
    def parseExternalSubset(self, ExternalID, SystemID):
        """DEPRECATED: Internal function, don't use.  parse Markup
          declarations from an external subset  [30] extSubset ::=
    ...
    def parseMisc(self):
        """DEPRECATED: Internal function, don't use.  parse an XML
           Misc* optional field.  [27] Misc ::= Comment | PI |  S """
    ...
    def parseName(self):
        """DEPRECATED: Internal function, don't use.  parse an XML
          ...
          ':' | CombiningChar | Extender  [5] Name ::= (Letter | '_'
           | ':') (NameChar)*  [6] Names ::= Name (#x20 Name)* """
    ...
    def parseNotationDecl(self):
        """DEPRECATED: Internal function, don't use.  Parse a notation
    ...
    def parsePEReference(self):
        """DEPRECATED: Internal function, don't use.  Parse a
    ...
    def parsePI(self):
        """DEPRECATED: Internal function, don't use.  parse an XML
          ...
          (Char* - (Char* '?>' Char*)))? '?>'  The processing is
           transferred to SAX once parsed. """
        libxml2mod.xmlParsePI(self._o)

    def parsePITarget(self):
        """DEPRECATED: Internal function, don't use.  parse the name
    ...
    def parseReference(self):
        """DEPRECATED: Internal function, don't use.  parse and handle
    ...
    def parseStartTag(self):
        """DEPRECATED: Internal function, don't use.  Parse a start
    ...
    def parseVersionNum(self):
        """DEPRECATED: Internal function, don't use.  parse the XML
          version value.  [26] VersionNum ::= '1.' [0-9]+  In
           practice allow [0-9].[0-9]+ at that level """
    ...
    def parserHandlePEReference(self):
        """DEPRECATED: Internal function, do not use.  [69]
    ...
    def skipBlankChars(self):
        """DEPRECATED: Internal function, do not use.  Skip whitespace
           in the input stream. """
    ...
    def switchEncodingName(self, encoding):
        """Use specified encoding to decode input data. This overrides
          the encoding found in the XML declaration.  This function
          can also be used to override the encoding of chunks passed
           to xmlParseChunk.  Available since 2.13.0. """
        ret = libxml2mod.xmlSwitchEncodingName(self._o, encoding)
        return ret
...
class xmlAttr(xmlNode):
    ...
    def removeProp(self):
        """Unlink and free an attribute including all children.  Note
          this doesn't work for namespace declarations.  The
           attribute must have a non-None parent pointer. """
    ...
    def addIDSafe(self, value):
        """Register a new id declaration  Available since 2.13.0. """
        ret = libxml2mod.xmlAddIDSafe(self._o, value)
        return ret
...
class xmlDtd(xmlNode):
    ...
    def dtdElementDesc(self, name):
        """Search the DTD for the description of this element  NOTE: A
...
class xmlEntity(xmlNode):
    ...
    def freeEntity(self):
        """Frees the entity. """
        libxml2mod.xmlFreeEntity(self._o)
...
class xmlNs(xmlNode):
    ...
    def newNodeEatName(self, name):
        """Create an element node.  Use of this function is
...
class outputBuffer(ioWriteWrapper):
    ...
    def write(self, len, data):
        ...
        ret = libxml2mod.xmlOutputBufferWrite(self._o, len, data)
        return ret
...
class inputBuffer(ioReadWrapper):
    ...
    def grow(self, len):
        """DEPRECATED: Internal function, don't use.  Grow up the
          content of the input buffer, the old data are preserved
          This routine handle the I18N transcoding to internal UTF-8
          This routine is used when operating the parser in normal
           (pull) mode """
        ret = libxml2mod.xmlParserInputBufferGrow(self._o, len)
        return ret

    def push(self, len, buf):
        """DEPRECATED: Internal function, don't use.  Push the content
          of the arry in the input buffer This routine handle the
          I18N transcoding to internal UTF-8 This is used when
           operating the parser in progressive (push) mode. """
        ret = libxml2mod.xmlParserInputBufferPush(self._o, len, buf)
        return ret

    def read(self, len):
        """DEPRECATED: Internal function, don't use.  Same as
...
class xmlTextReader(xmlTextReaderCore):
    ...
    def ByteConsumed(self):
        """DEPRECATED: The returned value is mostly random and
          useless. It reflects the parser reading ahead and is in no
          way related to the current node.  This function provides
          the current index of the parser used by the reader,
          relative to the start of the current entity. This function
          actually just wraps a call to xmlBytesConsumed() for the
          parser context associated with the reader. See
           xmlBytesConsumed() for more information. """
    ...
    def GetLastError(self):
        ...
        ret = libxml2mod.xmlTextReaderGetLastError(self._o)
        if ret is None:raise treeError('xmlTextReaderGetLastError() failed')
        __tmp = Error(_obj=ret)
        return __tmp
...
class ValidCtxt(ValidCtxtCore):
    ...
    def validateElement(self, doc, root):
        """Try to validate the subtree under an element """
        if doc is None: doc__o = None
        else: doc__o = doc._o
        if root is None: root__o = None
        else: root__o = root._o
        ret = libxml2mod.xmlValidateElement(self._o, doc__o, root__o)
        return ret
    ...
    def validatePushCData(self, data, len):
        """DEPRECATED: Internal function, don't use.  check the CData
           parsed for validation in the current stack """
...
class xpathContext:
    ...
    def xpathRegisterAllFunctions(self):
        """DEPRECATED: No-op since 2.14.0.  Registers all default
           XPath functions in this context """
...
class xpathParserContext:
    ...
    def xpathErr(self, code):
        """Handle an XPath error """
        libxml2mod.xmlXPathErr(self._o, code)

    def xpathEvalExpr(self):
        """DEPRECATED: Internal function, don't use.  Parse and
          evaluate an XPath expression in the given context, then
    ...
    def xpathValuePop(self):
        """Pops the top XPath object from the value stack """
        ret = libxml2mod.xmlXPathValuePop(self._o)
        if ret is None:raise xpathError('xmlXPathValuePop() failed')
        return xpathObjectRet(ret)
...
HTML_PARSE_HUGE = 524288
HTML_PARSE_IGNORE_ENC = 2097152
HTML_PARSE_BIG_LINES = 4194304
...
XML_SAVE_EMPTY = 256
XML_SAVE_NO_INDENT = 512
XML_SAVE_INDENT = 1024
